...
namespace TxF
{
    public class Directory
    {
        ...
        {
            PointerToDirectory = pointerToDirectory;
        }
        ...
        public static void CreateDirectory(string path, bool overwrite, Transaction transaction)
        {
            ...
            int err = apiwindows.CreateDirectoryTransactedW(null, path, new apiwindows.LPSECURITY_ATTRIBUTES(),
                transaction.TransactionHandle);
        ...
        ///     If this parameter is false and the directory doesn't exists, the method don't generate
        ///     exception
        /// </param>
        ...
        {
            try {
                int err = apiwindows.CreateSymbolicLinkTransactedW(pathSymbolicLinkDirectory, pathDirector,
                    apiwindows.TypeSymbolicLink.SYMBOLIC_LINK_FLAG_DIRECTORY, transaction.TransactionHandle);
                if (err == 0) {
                    throw new Win32Exception(Marshal.GetLastWin32Error());
                }
            }
