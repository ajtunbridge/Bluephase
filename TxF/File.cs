...
namespace TxF
{
    public class File
    {
        ...
        {
            var listFindFile = new List<apiwindows.WIN32_FIND_DATAW>();
            var find_dataw = new apiwindows.WIN32_FIND_DATAW();
            IntPtr result = IntPtr.Zero;

            try {
                result = apiwindows.FindFirstFileTransactedW(fileName, apiwindows.FINDEX_INFO_LEVELS.FindExInfoStandard,
                    out find_dataw, apiwindows.FINDEX_SEARCH_OPS.FindExSearchNameMatch, IntPtr.Zero, 0,
            ...
            var resultFiles = new string[listFindFile.Count];
            int i = 0;
        ...
        {
            int pbCancel = 0;
            var dwCopyFlags = apiwindows.COPY_FLAGS.COPY_FILE_COPY_ND;
            if (!overwrite) {
                dwCopyFlags = apiwindows.COPY_FLAGS.COPY_FILE_FAIL_IF_EXISTS;
            }
            int err = apiwindows.CopyFileTransactedW(sourceFileName, destFileName, null, IntPtr.Zero, ref pbCancel,
                dwCopyFlags, transaction.TransactionHandle);
        ...
        public static File CreateFile(string fileName, CreationDisposition creationDisposition, Transaction transaction)
        {
            IntPtr p = apiwindows.CreateFileTransactedW(fileName,
                ...
                apiwindows.FlagsAndAttributes.FILE_ATTRIBUTE_NORMAL |
                apiwindows.FlagsAndAttributes.FILE_ATTRIBUTE_ARCHIVE,
                IntPtr.Zero,
                transaction.TransactionHandle,
                null,
                IntPtr.Zero
        ...
        public static int WriteFile(File file, byte[] data)
        {
            try {
                int i = 0;
                int err = apiwindows.WriteFile(file.PointerToFile, data, data.Length, ref i,
                    new apiwindows.LPOVERLAPPED());
                if (err == 0) {
                    throw new Win32Exception(Marshal.GetLastWin32Error());
                }
                return err;
        ...
        public static void SetAttributes(string pathFile, FileAttributes fileAttributes, Transaction transaction)
        {
            ...
            int err = apiwindows.SetFileAttributesTransactedW(pathFile, (apiwindows.FILE_ATTRIBUTE) fileAttributes,
                transaction.TransactionHandle);
            if (err == 0) {
                throw new Win32Exception(Marshal.GetLastWin32Error());
            }
        ...
        {
            var win32_file_attribute_data = new apiwindows.WIN32_FILE_ATTRIBUTE_DATA();
            int err = apiwindows.GetFileAttributesTransactedW(pathFile,
                apiwindows.GET_FILEEX_INFO_LEVELS.GetFileExInfoStandard, out win32_file_attribute_data,
                transaction.TransactionHandle);
            if (err == 0) {
                throw new Win32Exception(Marshal.GetLastWin32Error());
            }
        ...
        {
            try {
                int err = apiwindows.CreateHardLinkTransactedW(pathHardLinkFile, pathFile,
                    new apiwindows.LPSECURITY_ATTRIBUTES(), transaction.TransactionHandle);
                if (err == 0) {
                    throw new Win32Exception(Marshal.GetLastWin32Error());
                }
            }
        ...
        {
            try {
                int err = apiwindows.CreateSymbolicLinkTransactedW(pathSymbolicLinkFile, pathFile,
                    apiwindows.TypeSymbolicLink.File, transaction.TransactionHandle);
                if (err == 0) {
                    throw new Win32Exception(Marshal.GetLastWin32Error());
                }
            }
